...
test('admin login with email+password returns token', async () => {
  await User.create({ email: 'admin@x.com', password_hash: bcrypt.hashSync('secret1', 10), role: 'admin' });
  const res = await request(createApp()).post('/api/v1/auth/login').send({ email: 'admin@x.com', password: 'secret1' });
  assert.strictEqual(res.status, 200);
  assert.ok(res.body.token);
  assert.strictEqual(res.body.role, 'admin');
});

test('affiliate login returns token', async () => {
